...
**Ghi chú về ảnh chụp Billing:** tại thời điểm chụp (12:56 ngày 14/08/2026), AWS Billing Dashboard hiển thị **USD 0.00 / "No data"**. Đây không phải sai sót khi thu thập mà do hai nguyên nhân: (1) AWS cập nhật dữ liệu billing trễ 6-24 giờ, trong khi hạ tầng mới chạy 1 giờ; (2) tài khoản đang ở **AWS Free Plan** nên phần chi phí phát sinh được credit miễn phí bù trừ. Cost Explorer API cũng trả về `DataUnavailableException` vì lý do tương tự. Bảng đơn giá phía trên được lấy từ bảng giá công bố của AWS cho region `us-east-1` để ước tính thay thế.
